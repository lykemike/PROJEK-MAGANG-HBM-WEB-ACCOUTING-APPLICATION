...
export default async (req, res) => {
  try {
    ...
    const update_header_penjualan = await prisma.headerPenjualan.updateMany({
      where: {
    ...
    const update_detail_penjualan = await prisma.detailPenjualan.update({
      where: {
    ...
    res.status(201).json([{ message: "Find produk detail success!", data: update_detail_penjualan }]);
  } catch (error) {
