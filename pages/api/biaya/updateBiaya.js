...
export default async (req, res) => {
  ...
  try {
    ...
    const update_header_biaya = await prisma.headerBiaya.createMany({
      where: {
    ...
    const delete_old_detail = await prisma.detailBiaya.deleteMany({
      where: {
        header_biaya_id: parseInt(req.body.no_transaksi),
      },
    });

    const delete_old_jurnal = await prisma.jurnalBiaya.deleteMany({
      where: {
        header_biaya_id: parseInt(req.body.no_transaksi),
      },
    });
