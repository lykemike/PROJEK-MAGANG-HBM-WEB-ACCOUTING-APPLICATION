...
import TabelNeraca from "../../components/Laporan/TabelNeraca";
...
export default function LaporanNeraca() {
  ...
  const [total_aset, setTotalAset] = useState(0);
  const [total_liabilitas, setTotalLiabilitas] = useState(0);
  ...
  useEffect(() => {
    Axios.post("/laporan/neraca", {
      ...
      .then(function (response) {
        setNeraca(response?.data?.data || []);
        setTotalAset(response?.data?.new_array[0]);
        console.log(response?.data?.new_array[0]);
      })
            ...
            <div variant="container">
              ...
              <TableContainer component={Paper}>
                ...
                <TableCell>
                  ...
                  {Neraca?.map((data, index) => (
                    <TabelNeraca key={index} data={data.value} label={data.label} label2={data.grandtotal} />
                  ))}
