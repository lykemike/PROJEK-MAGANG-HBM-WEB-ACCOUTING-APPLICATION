      ...
      <tr class='ml-4'>
        <td colSpan='3' class={open ? "block" : "hidden"}>
          <table class='w-full ml-4'>
            ...
            />
             <KirimUang 
                tgl_transaksi={data.JurnalKirimUang[0]?.header_kirim_uang.tgl_transaksi || '-'}
                no_transaksi={data.JurnalKirimUang[0]?.header_kirim_uang.no_transaksi || '-'}
                nominal={data.JurnalKirimUang[0]?.nominal || '0'}
                tipe_saldo={data.JurnalKirimUang[0]?.tipe_saldo || '-'}
                />
            <TerimaUang 
                tgl_transaksi={data.JurnalTerimaUang[0]?.header_terima_uang.tgl_transaksi || '-'}
                no_transaksi={data.JurnalTerimaUang[0]?.header_terima_uang.no_transaksi || '-'}
                nominal={data.JurnalTerimaUang[0]?.nominal || '0'}
                tipe_saldo={data.JurnalTerimaUang[0]?.tipe_saldo || '-'}
             />
             <TransferUang 
                tgl_transaksi={data.JurnalTransferUang[0]?.transfer_uang.tgl_transaksi || '-'}
                no_transaksi={data.JurnalTransferUang[0]?.transfer_uang.no_transaksi || '-'}
            ...
            <tfoot class='border-t-2 border-black'>
              <tr>
                ...
                <td class='px-2 py-1'>
                  Rp. {(data.DetailJurnal && data.JurnalPenjualan && data.JurnalKirimUang && data.JurnalTerimaUang && data.JurnalTransferUang).reduce((a, b) => (a = a + b.debit), 0).toLocaleString({ minimumFractionDigits: 0 })}</td>
