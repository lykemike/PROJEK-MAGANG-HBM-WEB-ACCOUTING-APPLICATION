...
export default function Test2({ data, labaKotor, pendapatanBersihOperasional, pendapatanBersihSebelumPajak, pendapatanBersihSesudahPajak }) {
  let value = data?.filter((i) => i.label == "Beban Selain Beban Pajak");
  console.log(value);
    ...
    <>
      <TableBody>
        ...
        <TableRow>
          ...
          <TableCell align="right" />
          <TableCell align="right">
            <Typography variant="h6">{labaKotor == 0 ? "Rp. 0, 00" : "Rp. " + labaKotor.toLocaleString({ minimumFractionDigits: 0 })}</Typography>
          </TableCell>
        </TableRow>
